...
export default class PageStack {
  ...
  createPage(node, state) {
    ...
    const pn = new PageNode(cloneVNode(node, { key: ((_b = (_a = node.props) == null ? void 0 : _a.key) == null ? void 0 : _b.toString()) + tag }), tag);
  ...
  copyKeyProps(page, newNode) {
    return cloneVNode(newNode, { key: page.node.key });
  }
  ...
  _evaluate(n, ctx) {
    ...
    if (this.lastDisplayPage && this.lastDisplayPage.state) {
      const { position: targetPosition, curNode } = state;
